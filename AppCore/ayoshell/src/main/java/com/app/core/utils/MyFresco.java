...
public class MyFresco {
    ...
    private static MyFresco instance;

    private ExecutorService executeBackgroundTask = Executors.newSingleThreadExecutor();

    public static MyFresco getDefault() {
        if (instance == null) {
            instance = new MyFresco();
        }
        return instance;
    }
    ...
    private void fetch(final Uri uri, final FrescoBitmapCallback<Bitmap> callback){
     ...
     * @param <T> T
     */
    private <T> void postResult(final T result, final Uri uri,
            final FrescoBitmapCallback<T> callback) {
    ...
    public static void setImageUri(final ImageView iv, String uri){
        if(Lang.isEmpty(uri)) return;
        if(iv == null) return;
        Uri u = Uri.parse(uri);
        MyFresco.getDefault().fetch(u, new FrescoBitmapCallback<Bitmap>() {
            @Override
