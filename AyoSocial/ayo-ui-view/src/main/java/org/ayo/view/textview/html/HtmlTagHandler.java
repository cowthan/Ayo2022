...
public abstract class HtmlTagHandler {
    ...
    public static Spanned fromHtml(String source, Html.ImageGetter getter, HtmlTagHandler tagHandler){
        HtmlToSpannedConverter h = new HtmlToSpannedConverter(source, getter, tagHandler);
        Spanned s = h.convert();
        return s;
    }

    public static void setHtml(TextView tv, String source){
        if(tv == null) return;
        if(source == null || source.isEmpty()) tv.setText("");
        HtmlToSpannedConverter h = new HtmlToSpannedConverter(source, null, new SpanFontColorAndSizeHandler());
        Spanned s = h.convert();
        tv.setText(s);
    }

    public abstract void handleTag(boolean open, String tag, Editable output, Attributes attrs);
//    public interface AyoTagHandler {
//
//    }

    public static class SpanFontColorAndSizeHandler extends HtmlTagHandler {
                ...
                if(open){
                    ...
                    for(int i = 0; i < attrs.getLength(); i++){
                        if(attrs.getLocalName(i).equals("style")){
                            //style="color:#e60012;font-size:15sp";
    ...
    static class HtmlToSpannedConverter implements ContentHandler {
        ...
        private HtmlTagHandler mTagHandler;

        public HtmlToSpannedConverter(String source, Html.ImageGetter imageGetter, HtmlTagHandler taghandler) {
            mSource = source;
