...
/**
 *  这个基类处理了linear，grid，stargger的header问题，方式各不相同，参考：
 *  http://www.tuicool.com/articles/qMnAfen
 *
 *  linear靠adapter的type
 *  grid靠type和span lookup
 *  stagger没有lookup，靠type和setFullSpan
 *
 */
public abstract class BaseRecyclerAdapter<T, V extends RecyclerView.ViewHolder> extends RecyclerView.Adapter<V> {
    ...
    public V onCreateViewHolder(ViewGroup parent, final int viewType) {
        if(mHeaderView != null && viewType == TYPE_HEADER) return createHolder(mHeaderView);
    ...
    protected abstract V createHolder(View itemView);
