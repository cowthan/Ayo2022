...
package com.app.core.recycler.divider;
...
public abstract class FlexibleDividerDecoration extends RecyclerView.ItemDecoration {
    ...
    private boolean hasVerticalDivider(RecyclerView parent, int position) {
        ...
        if (parent.getLayoutManager() instanceof GridLayoutManager) {
            ...
            StaggeredGridLayoutManager manager = (StaggeredGridLayoutManager)parent
                    .getLayoutManager();
            StaggeredGridLayoutManager.LayoutParams params = (StaggeredGridLayoutManager.LayoutParams)manager
                    .findViewByPosition(position).getLayoutParams();
            int spanCount = manager.getSpanCount();
            ...
            if (manager.getOrientation() == OrientationHelper.VERTICAL) {
                ...
                for (int p : lastPosition) {
                    if (p != position && p != -1) {
                        StaggeredGridLayoutManager.LayoutParams params1 = (StaggeredGridLayoutManager.LayoutParams)manager
                                .findViewByPosition(p).getLayoutParams();
    ...
    /**
     * In the case mShowLastDivider = false, Returns offset for how many views
     * we don't have to draw a divider for, for LinearLayoutManager it is as
     * simple as not drawing the last child divider, but for a GridLayoutManager
     * it needs to take the span count for the last items into account until we
     * use the span count configured for the grid.
     *
     * @param parent RecyclerView
     * @return offset for how many views we don't have to draw a divider or 1 if
     *         its a LinearLayoutManager
     */
    ...
    public interface VisibilityProvider {
         ...
         *
         * @param position Divider position (or group index for
         *            GridLayoutManager)
         * @param parent RecyclerView
