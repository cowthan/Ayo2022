...
public class AdapterDelegatesManager<T extends ItemBean> {
    ...
    public AdapterDelegatesManager<T> addDelegate(@NonNull AyoItemTemplate<T> delegate) {
        ...
        // but it's very unlikely that we reach Integer.MAX_VALUE and run out of
        // unused indexes
    ...
    public AdapterDelegatesManager<T> addDelegate(int viewType, boolean allowReplacingDelegate,
        ...
        if (viewType == FALLBACK_DELEGATE_VIEW_TYPE) {
            throw new IllegalArgumentException("The view type = " + FALLBACK_DELEGATE_VIEW_TYPE
    ...
    public AyoViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        ...
        if (vh == null) {
            throw new NullPointerException("ViewHolder returned from AdapterDelegate " + delegate
    ...
    public void onBindViewHolder(@NonNull T items, int position,
        ...
        onBindViewHolder(items, position, viewHolder, viewHolder.getItemViewType());
    }

    public void onBindViewHolder(@NonNull
    final T items, final int position, @NonNull
    final AyoViewHolder viewHolder, int viewType) {

        final AyoItemTemplate<T> delegate = delegates.get(viewType);
        if (delegate == null) {
            throw new NullPointerException("No AdapterDelegate added for ViewType " + viewType);
        }
        Log.e("adapter",
                "onBindViewHolder()    delate class: " + delegate.getClass().getSimpleName());
        delegate.onBindViewHolder(items, position, viewHolder);

        if (delegate.getOnItemClickCallback() != null) {
            viewHolder.root().setOnClickListener(new View.OnClickListener() {
                @Override
                public void onClick(View v) {
                    if (delegate.getOnItemClickCallback() != null) {
                        delegate.getOnItemClickCallback().onItemClick(null, viewHolder.root(),
                                position, items);
                    }
                }
            });
