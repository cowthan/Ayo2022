...
import org.ayo.list.pinned.OnHeaderClickAdapter;
import org.ayo.list.pinned.SmallPinnedHeaderItemDecoration;
...
public class PinnedSecondActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.pinned_activity_second);
        ...
        mAdapter = new BaseHeaderAdapter<PinnedHeaderEntity<Integer>>(data) {
            ...
            protected void addItemTypes() {
                addItemType(BaseHeaderAdapter.TYPE_HEADER, R.layout.pinned_item_data_with_small_pinned_header);
                addItemType(BaseHeaderAdapter.TYPE_DATA, R.layout.pinned_item_data);
            }
            ...
            protected void convert(BaseViewHolder holder, PinnedHeaderEntity<Integer> item) {
                switch (holder.getItemViewType()) {
                    case BaseHeaderAdapter.TYPE_HEADER:
                        Glide.with(PinnedSecondActivity.this).load(item.getData()).into((ImageView) holder.getView(R.id.iv_small_pinned_header));
                        Glide.with(PinnedSecondActivity.this).load(item.getData()).into((ImageView) holder.getView(R.id.iv_animal));
            ...
            public void SimpleOnItemChildClick(BaseQuickAdapter baseQuickAdapter, View view, int i) {
                ...
                switch (view.getId()) {
                    case R.id.iv_small_pinned_header:
                        Toast.makeText(PinnedSecondActivity.this, "click tag: " + entity.getPinnedHeaderName(), Toast.LENGTH_SHORT).show();
                        break;
                    case R.id.iv_animal:
                        Toast.makeText(PinnedSecondActivity.this, entity.getPinnedHeaderName() + ", position " + i + ", id " + entity.getData(), Toast.LENGTH_SHORT).show();
                        break;
                }
            }
        });
        ...
        OnHeaderClickAdapter headerClickAdapter = new OnHeaderClickAdapter() {
            ...
            public void onHeaderClick(View view, int id, int position) {
                if (id == R.id.iv_small_pinned_header) {
                    Toast.makeText(PinnedSecondActivity.this, "click tag: " + mAdapter.getData().get(position).getPinnedHeaderName(), Toast.LENGTH_SHORT).show();
                }
            }
        };
        mRecyclerView.addItemDecoration(new SmallPinnedHeaderItemDecoration.Builder(R.id.iv_small_pinned_header, BaseHeaderAdapter.TYPE_HEADER).enableDivider(true)
                .setDividerId(R.drawable.pinned_section_divider).disableHeaderClick(false).setClickIds(R.id.iv_small_pinned_header).setHeaderClickListener(headerClickAdapter).create());
