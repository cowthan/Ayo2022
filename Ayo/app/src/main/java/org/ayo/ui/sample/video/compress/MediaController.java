...
import org.ayo.ui.sample.video.compress.video.InputSurface;
import org.ayo.ui.sample.video.compress.video.MP4Builder;
import org.ayo.ui.sample.video.compress.video.Mp4Movie;
import org.ayo.ui.sample.video.compress.video.OutputSurface;
...
public class MediaController {
     ...
     */
    @TargetApi(16)
    public boolean convertVideo(CompressListener listener, String videoPath,
                                long startTime,
                                long endTime,
                                String resultPath,
                                int resultWidth,
        ...
        Log.e("视频处理", "转换视频\n" + str);
        SharedPreferences preferences = AppCore.app().getSharedPreferences("videoconvert", Activity.MODE_PRIVATE);
...
// 故改为使用createByCodecName("OMX.SEC.aac.enc")创建音频编码器。估计这是一个API bug。
                            ...
                            while (!outputDone) {
                                ...
                                if (sampleTime < totalMicroseconds && listener != null) {
                                    listener.compressProgress(totalMicroseconds, sampleTime);
                                }
                                checkConversionCanceled();
                                if (!inputDone) {
                                    boolean eof = false;
    ...
    public void release() {
        if (extractor != null && Build.VERSION.SDK_INT >= Build.VERSION_CODES.JELLY_BEAN) {
            extractor.release();
        }
    }
}
