...
public class MainRecorderActivity extends Activity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.ac_recorder_main);
    ...
    private void openOutFile(){
        ...
        if(outFile == null || !outFile.exists()){
            Toaster.toastShort("文件不存在：" + (outFile == null ? "null" : outFile.getAbsolutePath()));
            return;
        }

        Trace.e("video", outFile.getAbsolutePath());
        ...
        if (Build.VERSION.SDK_INT < Build.VERSION_CODES.N) {
            ...
            uri = FileProvider.getUriForFile(this, AppCore.app().getPackageName() + ".fileProvider", outFile);
        }
        //pdf文件要被读取所以加入读取权限
    ...
    private void sendOutFile(){
        try{
            ...
            //关于MIME：http://blog.csdn.net/gjy211/article/details/72626233
            imageIntent.setType("video/mp4");

            if (Build.VERSION.SDK_INT < Build.VERSION_CODES.N) {
                imageIntent.putExtra(Intent.EXTRA_STREAM, Uri.fromFile(outFile));
            } else {
                imageIntent.setFlags(Intent.FLAG_GRANT_READ_URI_PERMISSION);
                Uri contentUri = FileProvider.getUriForFile(this, AppCore.app().getPackageName() + ".fileProvider", outFile);
